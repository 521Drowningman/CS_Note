...
```c
char *word;
// word[1] 相当于 *(word + 1)  这是一个字符而不是字符串
```
...
> Makefile 会读取一个C文件,然后调用gcc编译器,生成一个 .s文件(汇编文件),然后由汇编解释器生成 .o 文件.
>
> Makefile 会为所有的内核文件做相同的操作,最后由系统加载器链接所有的 .o 文件,生成内核文件.

xv6启动过程:

从_entry开始, _entry调用start函数,start将整个系统从机器模式转入管理模式,start返回到main函数；

main函数开始初始化设备和子系统,然后调用userinit创建第一个进程,
